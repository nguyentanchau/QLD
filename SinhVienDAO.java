...
public class SinhVienDAO implements ISinhVienDAO {
    ...
    public SinhVien addNew(SinhVien sv) {
        ...
        if (DBConnect.open()) {
            try {
                ...
                ps = DBConnect.cnn.prepareStatement("insert into SinhVien(MaSV,HoTen,MaLop,HeDaoTao,NgaySinh,DiaChi,GioiTinh,SDT) values (?,?,?,?,?,?,?,?)");
    ...
    public SinhVien updateByID(SinhVien sv) {
        ...
        if (DBConnect.open()) {
            try {
                ps = DBConnect.cnn.prepareStatement("update SinhVien set HoTen = ?, MaLop = ?, HeDaoTao = ?, NgaySinh = ?, DiaChi = ?, GioiTinh = ?, SDT = ? where MaSV = ?");
    ...
    public static void main(String[] args) {
        System.out.println(new SinhVienDAO().findByIDLop("").get(0).getMalop());
