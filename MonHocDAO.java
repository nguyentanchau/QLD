...
public class MonHocDAO implements IMonHocDAO {
    ...
    public ArrayList<MonHoc> getAll() {
        ...
        if (DBConnect.open()) {
            try {
                ps = DBConnect.cnn.prepareStatement("select *from MonHoc");
                rs = ps.executeQuery();
                list = new ArrayList<>();
                while (rs.next()) {
    ...
    public ArrayList<MonHoc> findIDMaMH(String maMH) {
        ...
        if (DBConnect.open()) {
            try {
                ps = DBConnect.cnn.prepareStatement("select *from MonHoc where MaMH =?");
                ps.setString(1, maMH);
                rs = ps.executeQuery();
                list = new ArrayList<>();
                while (rs.next()) {
    ...
    public MonHoc addNew(MonHoc mh) {
        ...
        if (DBConnect.open()) {
            try {
                ps = DBConnect.cnn.prepareStatement("insert into MonHoc(MaMH, TenMH,SoTrinh, HinhThucThi,HocKy,PhongHoc) values (?,?,?,?,?,?)");
    ...
    public MonHoc updateByID(MonHoc mh) {
        ...
        if (DBConnect.open()) {
            try {
                ps = DBConnect.cnn.prepareStatement("update MonHoc set TenMH=?,SoTrinh=?, HinhThucThi =?,HocKy =?,PhongHoc =? where MaMH =?");
    ...
    public void delMonHoc(String MonHocID) throws SQLException, ClassNotFoundException {
        PreparedStatement ps = null;
        if (DBConnect.open()) {
            ps = DBConnect.cnn.prepareStatement("delete from MonHoc where MaMH= ?");
    ...
    public ArrayList<MonHoc> CheckID(String mamh) {
       // throw new UnsupportedOperationException("Not supported yet.");
        ...
        if (DBConnect.open()) {
            try {
                psCheck = DBConnect.cnn.prepareStatement("select *from MonHoc where MaMH=?");
